
public class Date {
	int apa_rece; /*valoarea consumului de apa rece pe luna curenta */
	int apa_calda; /*valoarea consumului de apa calda pe luna curenta*/
	int energ; /*valoarea consumului de curent pe bloc repartizat pe locatar*/
	int rep; /*fondul de reparatii per locuinta*/
	int suma; /*intretinerea pe luna curenta*/
	int sal; /*suma de bani preluata de la fiecare apartament in parte pentru a 
	           plati serviciile de curat*/
    int nr_ap; /*numar persoane in apartament*/
    int nr_loc; /*numar locuitori din apartament*/
}
